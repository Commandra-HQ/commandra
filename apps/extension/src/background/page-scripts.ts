...
export function clickInPage(
  ...
  function findElement(
    ...
    for (const c of candidates) {
      ...
      if (!cl) continue;
      if (cl === ll)
        return { element: c, usedSelector: 'fuzzy:exact', method: 'fuzzy' };
      let score = 0;
    ...
    if (bestMatch)
      return {
        element: bestMatch,
        usedSelector: `fuzzy:${bestScore.toFixed(2)}`,
        method: 'fuzzy',
      };
  ...
  if (!(el instanceof HTMLElement))
    return { success: false, error: `Not clickable: ${selector}` };
...
export function typeInPage(
  ...
  const {
    ...
    usedSelector,
    method,
  } = findElement(selector, fallbacks, label, 'input');
  if (!el) return { success: false, error: `Element not found: ${selector}` };
  ...
  if (
    ...
    return {
      success: false,
      error: `Not a text input or contenteditable element: ${selector}`,
    };
  }
  el.value = '';
...
export function selectInPage(
  ...
  const {
    ...
    usedSelector,
    method,
  } = findElement(selector, fallbacks, label, 'select');
  if (!el) return { success: false, error: `Element not found: ${selector}` };
  ...
  return {
    success: true,
    data: { selected: value, selector: usedSelector, method },
  };
}
...
export function getPageStateInPage() {
  ...
  document.querySelectorAll(interactiveSelectors).forEach(el => {
    ...
    if (el.id) selector = `#${el.id}`;
      ...
      selector = `${type}[name="${el.getAttribute('name')}"]`;
...
export function readTextInPage(
  selector: string,
  allStr: string,
  maxLengthStr: string,
) {
...
export function readTableInPage(
  selector: string,
  maxRowsStr: string,
  includeLinksStr: string,
) {
...
export function readDataGrid(
  ...
  if (gridRows.length === 0) {
    return {
      success: false,
      error: `No table or data grid found at: ${selector}`,
    };
  }
  ...
  for (let i = 1; i < gridRows.length; i++) {
    ...
    const cells = gridRows[i].querySelectorAll(
...
export function refreshPageStateInPage() {
  ...
  else if (path.match(/\/\d+$/) || path.match(/\/[a-f0-9-]{36}$/))
    pageType = 'detail';
