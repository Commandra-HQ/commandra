...
export class OpenAIProvider implements LLMProvider {
	...
	async *chat(params: ChatParams): AsyncIterable<StreamEvent> {
		...
		const functionCalls = new Map<string, { callId: string; name: string; json: string }>();
			...
			switch (event.type) {
				...
				case 'response.output_item.added': {
					...
					if (item.type === 'function_call') {
						const fc = item as Responses.ResponseFunctionToolCall & { id?: string };
						// item_id in delta events = the item's `id`, NOT `call_id`
						const itemId = fc.id || fc.call_id;
						console.log(`[OpenAI] function_call added: itemId=${itemId} callId=${fc.call_id} name=${fc.name}`);
						functionCalls.set(itemId, { callId: fc.call_id, name: fc.name, json: '' });
	...
	for (const m of messages) {
		if (typeof m.content === 'string') {
			...
			if (m.role === 'assistant') {
				...
				const otherBlocks = blocks.filter((b) => b.type !== 'tool_result' && b.type !== 'thinking');
