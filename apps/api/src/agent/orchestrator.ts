...
export async function runOrchestrator(
  ...
  while (iterations < maxIterations) {
    ...
    const usedChars =
      estimateMessageChars(currentMessages) + systemPrompt.length;
...
async function autoCompact(
  ...
  try {
    const transcript = currentMessages
      .map(
        m =>
          `${m.role}: ${typeof m.content === 'string' ? m.content : '[tool data]'}`,
      )
      .join('\n\n');
    ...
    if (summary) {
      const { path } = await saveCompaction(
        userId,
        conversationId,
        transcript,
        summary,
...
async function streamLLMResponse(
  ...
  try {
    for await (const event of streamIter) {
      if (signal?.aborted)
        return { text, stopReason, contextError: false, aborted: true };
      ...
      switch (event.type) {
        case 'thinking_delta':
          ...
          if (
            ...
            (content[content.length - 1] as ThinkingContentBlock).thinking +=
              event.text;
          } else {
        ...
        case 'thinking_signature': {
          ...
          const lastThinking = [...content]
            .reverse()
            .find(b => b.type === 'thinking');
          if (lastThinking)
            (lastThinking as ThinkingContentBlock).signature = event.signature;
          break;
        }
        case 'text':
          textChunks++;
          ...
          if (
            ...
            (content[content.length - 1] as { text: string }).text +=
              event.text;
          } else {
          ...
          if (b.type === 'tool_result') {
            ...
            if (Array.isArray(tr.content)) {
              return {
                ...
                content: tr.content
                  .filter(
                    (sub: TextBlock | { type: string }) => sub.type !== 'image',
                  )
...
async function processToolCalls(
  ...
  if (partitioned.safe.length > 0) {
    console.log(
      `[Orchestrator] Executing ${partitioned.safe.length} safe tools in parallel`,
    );
  ...
  for (const block of partitioned.blocked) {
    toolResults.push({
      ...
      content: JSON.stringify({
        success: false,
        error:
          'This action is blocked by safety policy. Ask the user to confirm explicitly.',
      }),
      isError: true,
    });
  }
  ...
  toolResults.sort(
    (a, b) =>
      (orderMap.get(a.toolUseId) ?? 0) - (orderMap.get(b.toolUseId) ?? 0),
  );
