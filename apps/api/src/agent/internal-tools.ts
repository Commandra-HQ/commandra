...
import { readScratchpad, writeScratchpad } from '../storage/scratchpad.js';
import { buildSitemapFromDB } from '../storage/sitemap.js';
...
export async function executeInternalTool(
	...
	switch (block.name) {
		...
		case 'build_sitemap':
			return handleBuildSitemap(block, ctx);
		case 'list_tabs':
...
async function handleRecallMemory(
	...
	try {
		const results = await searchUserMemories(args.query, ctx.userId, ctx.domain, 5);
		const formatted =
			results.length > 0
				? results
...
async function resolveWriteMode(
	...
	if (mode === 'merge') {
		...
		if (existingEntries.length > 0 && newEntries.length > 0) {
			...
			const capped =
				allEntries.length > MAX_FILE_LINES
...
async function handleSaveToLocal(
	...
	try {
		const saved = saveLocalFile(args.category, ctx.domain, args.filename, args.content);
		await ctx.onEvent({
			type: 'tool_start',
...
async function handleUpdateAgentFiles(
	...
	try {
		await uploadAgentFile(ctx.userId, args.agentSlug, args.filename, args.content);
		await ctx.onEvent({
			type: 'tool_start',
...
async function handleSubmitPlan(
	...
	try {
		...
		const autoApprovePlan = ctx.autonomy === 'trusted' || ctx.autonomy === 'autonomous';
...
async function handleReadLocalFile(
	...
	try {
		...
		const content =
			result.length > maxBytes ? result.slice(0, maxBytes) + '\n...[truncated]' : result;
...
async function handleScheduleAgent(
	...
	if (!args.runAt && !args.cron) {
		return errorResult(
			block.id,
			new Error(
				'Either runAt (ISO datetime for one-time) or cron (expression for recurring) is required',
			),
		);
	}
	...
	try {
		...
		const [agent] = await db.select().from(agents).where(eq(agents.slug, args.agentSlug)).limit(1);

		if (!agent) {
			return errorResult(
				block.id,
				new Error(`Agent "${args.agentSlug}" not found. Check the slug.`),
			);
		}
		...
		if (args.runAt) {
			...
			if (isNaN(runAt.getTime())) {
				return errorResult(
					block.id,
					new Error(`Invalid runAt datetime: "${args.runAt}". Use ISO 8601 format.`),
				);
			}
...
async function handleBuildSitemap(
	block: ToolUseBlock,
	ctx: InternalToolContext,
): Promise<ToolResultBlock> {
	const args = block.input as { domain?: string };
	const domain = args.domain || ctx.domain;

	if (!domain) {
		return errorResult(
			block.id,
			new Error('No domain specified and no current page domain available.'),
		);
	}

	try {
		const result = await buildSitemapFromDB(ctx.userId, domain);
		return successResult(block.id, result);
	} catch (err) {
		return errorResult(block.id, err);
	}
}
