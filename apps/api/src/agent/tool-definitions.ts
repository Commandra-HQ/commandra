...
export function buildToolList(
	...
	const readLocalFileTool = {
		...
		parameters: {
			...
			properties: {
				path: {
					type: 'string',
					description: 'Path relative to ~/.commandra/ (e.g. "exports/mail.google.com/report.csv")',
				},
				maxBytes: {
					type: 'number',
		...
		parameters: {
			...
			properties: {
				agentSlug: {
					type: 'string',
					description:
						'Slug of the agent to schedule (e.g. "gmail-sender"). Use the coordinator if no specific agent exists.',
				},
				task: {
					type: 'string',
				...
				runAt: {
					type: 'string',
					description:
						'ISO 8601 datetime for one-time task (e.g. "2026-03-25T15:00:00"). Use this for "in 15 minutes" or "at 3pm" type requests.',
				},
				cron: {
					type: 'string',
					description:
						'Cron expression for recurring schedule (e.g. "0 15 * * *" = 3pm daily). Only use for recurring tasks, not one-time.',
				},
			},
	...
	return [
		...
		...(currentDepth >= (agentConfig.limits?.maxDepth ?? 2)
			? []
			: [spawnAgentTool, waitForAgentsTool]),
		saveToLocalTool,
		createAgentTool,
		...
		listLocalFilesTool,
		{
			name: 'build_sitemap',
			description:
				'Build or rebuild the site navigation graph (SITEMAP.yaml) for the current domain. Reads all indexed pages from the database and constructs a graph of pages (nodes) and navigation paths (edges). Use this after visiting multiple pages to create or update the site map, or when the sitemap seems incomplete. The sitemap is then injected into your prompt on future runs and visible in the dashboard graph view.',
			parameters: {
				type: 'object' as const,
				properties: {
					domain: {
						type: 'string',
						description:
							'The domain hostname to build the sitemap for (e.g. "localhost", "mail.google.com"). Defaults to the current page domain.',
					},
				},
				required: [],
			},
